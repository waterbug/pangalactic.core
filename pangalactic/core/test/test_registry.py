...
# the generated ontology documentation, which lives at the top of the source
# tree and is therefore absent from an installed package
ONTOLOGY_DOC = os.path.join(pangalactic.core.__path__[0], '..', '..', 'doc',
                            'pgef_ontology.html')


class RegistryTests(unittest.TestCase):

    def test_00_get_nearest_persistable_base(self):
        """
        CASE:  get_nearest_persistable_base

        Tests the output for a few typical cases.
        """
        value = [r.get_nearest_persistable_base('HardwareProduct'),
                 r.get_nearest_persistable_base('Product')]
        expected = ['Product', 'ManagedObject']
    ...
    def test_05__update_schemas_from_extracts(self):
        ...
        * N.B.:  depends on success of C{test_03__create_extracts_from_source}.
        ...
        value_1 = len([e for e in list(r.ces) if e not in list(r.schemas)])
        expected_1 = 0
        # FIXME!
    ...
    # FIXME!
    # def test_06_create_classes_from_src(self):
        # """
        # CASE:  create_classes_from_src

        # Compares the return value of create_metaobjects_from_src with a set of
        # meta objects corresponding to the source's model.
        # """
        # # use space_mission.owl

    @unittest.skipUnless(os.path.exists(ONTOLOGY_DOC),
                         'generated ontology doc not present (installed '
                         'package rather than source tree)')
    def test_99_ontology_doc_is_current(self):
        """
        CASE:  every class in the ontology appears in the generated HTML
        documentation.

        The documentation is generated by registry.report_html() and
        committed, so it goes stale silently whenever the ontology gains a
        class and nobody regenerates it -- which is what happened to
        "CheckOut", missing from it from schema version 3.4.0 until 3.5.0.
        Regenerate with:

            orb.start(home=...)
            open('doc/pgef_ontology.html', 'w').write(orb.registry.report_html())
        """
        doc = open(ONTOLOGY_DOC).read()
        expected = []
