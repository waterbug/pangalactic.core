# TO DO List for Pangalactic Gargleblaster

Highest priority development items.  The list is in approximate priority order,
but priorities are subject to change.

* Tutorial videos
  - a basic training video
  - a set of focused videos for various aspects and processes

* Interface to 42 (ACS simulation tool)
  - maintain ACS system component models in 42-compatible formats
...
* Hardware Library:
  - Add "Only Non-Project Specs" filter to HW library (filters out
    Project-owned items)
  - Develop method to find and scrape product data sheets and add library
    components (possibly including AI support)
...
Other External Interfaces (priorities TBD):
...
Any item with a conceptual basis gets a `NOTES_ON_*.md` design note of its own
and is referenced from here rather than described at length.

This list is maintained in `pangalactic.core` because it spans the repositories
and belongs under version control;  it lived in the author's sandbox until
2026-08-22.
